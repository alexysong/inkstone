...
def s_1l(thickness, ql, al0, bl0, gb = gb):
    ...
    a = al0
    b = bl0
    # alu = gb.sla.lu_factor(a)
    aTlu = gb.sla.lu_factor(a.T)
    ba_ = (gb.sla.solve(a.T, b.T)).T

    fl = gb.exp(1j * ql * thickness)

    # a_fbafb = a - fl[:, None] * b @ gb.sla.lu_solve(alu, (fl[:, None] * b))
    a_fbafb = a - fl[:, None] * ba_ @ (fl[:, None] * b)
    a_fbafb_lu = gb.sla.lu_factor(a_fbafb)

    # fbafa_b = (fl[:, None] * b @ gb.sla.lu_solve(alu, fl[:, None] * a) - b)
    fbafa_b = fl[:, None] * ba_ @ (fl[:, None] * a) - b
    # fa_bab = (fl[:, None] * (a - b @ gb.sla.lu_solve(alu, b)))
    fa_bab = (fl[:, None] * (a - ba_ @ b))

    s11 = gb.sla.lu_solve(a_fbafb_lu, fbafa_b)
    s12 = gb.sla.lu_solve(a_fbafb_lu, fa_bab)
...
def s_1l_rsp(thickness: float,
    ...
    ab = gb.sla.lu_solve(alu, b)
    ba = gb.sla.lu_solve(aTlu, b.T).T
    bfa = gb.sla.lu_solve(aTlu, (b*f).T).T
    ...
    a_bfabf = a - b * f @ ab * f
    # try:
    term = gb.sla.solve(a_bfabf.T, a_babf.T).T
    ...
    #         term = gb.sla.solve((a_bfabf+1e-14 * gb.eye(len(ql))).T, (a_babf+1e-14*gb.eye(len(ql))).T).T
    #         print(la.cond(term))
    #     else:
    #         return e
    ...
    # alu = gb.sla.lu_factor(a)
    # aib = gb.sla.lu_solve(alu, b)
    # sl11 = b @ ia
    sl11 = gb.sla.lu_solve(aTlu, b.T).T
    # sl12 = a - b @ ia @ b
    sl12 = 0.5 * (a - b @ ab)
    sl21 = aTlu2
    sl22 = -ab
    ...
    # ts[4] *= 1e9
    #
    # term_ts = gb.sla.solve((a_bfabf*ts).T, (a_babf*ts).T).T
    # diff = term - term_ts
    ...
    # sngl1 = (a - b @ ab)*f
    # sngl2 = a - b * f @ ab * f
    # prdc = gb.sla.solve(sngl2.T, sngl1.T).T
    # prdc1 = prdc.copy()
    # prdc1[gb.where(gb.abs(prdc) < 1e-14)] = 0.
    # prdctest = gb.sla.solve(sngl1.T, sngl1.T).T

    # # debugging
    # # wong left/right scattering matrix
    # f = gb.exp(1j * ql * 0.5)
    # sngl1 = f * (a - b @ ab)
    # sngl2 = gb.sla.solve(b.T, a).T - f * gb.sla.solve(a.T, b).T * f
    # prdc = gb.sla.solve(sngl2, sngl1)
    # prdc1 = prdc.copy()
    # prdc1[gb.where(gb.abs(prdc) < 1e-14)] = 0.
    # # prdctest = gb.sla.solve(sngl1.T, sngl1.T).T
...
def s_1l_rsp_lrd(thickness: float,
    ...
    # ia = la.inv(a)
    a2Tlu = gb.sla.lu_factor(a2.T)
    a2Tlu2 = (gb.clone(a2Tlu[0]), gb.clone(a2Tlu[1]))
    a1 = a2Tlu2[0]
    a1 = gb.assignAndMultiply(a1, gb.triu_indices(a1.shape[0]), 0.5)
    a2lu = gb.sla.lu_factor(a2)
    # alu2 = (alu[0].copy(), alu[1].copy())
    # a1 = alu2[0]
    # a1 = gb.assignAndMultiply(a1, gb.triu_indices(a1.shape[0]), 0.5)

    a2b2 = gb.sla.lu_solve(a2lu, b2)

    sr11 = -a2b2
    sr12 = a2Tlu2
    sr21 = 0.5 * (a2 - b2 @ a2b2)
    sr22 = gb.sla.lu_solve(a2Tlu, b2.T).T
...
def s_1l_1212(a, b):
    ...
    s11 = gb.sla.lu_solve(aTlu, b.T).T
    s12 = 1./2. * (a - b @ ab)
    s21 = alu2
    s22 = - ab
...
def s_1l_1221(a, b):
    ...
    s12 = alu2
    s21 = 1./2. * (a - b @ ab)
    s22 = gb.sla.lu_solve(aTlu, b.T).T
