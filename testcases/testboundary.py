...
num_g = 30
eps_vals = torch.logspace(-10,-1,100)
...
f = bk.data(10.,requires_grad=True)
z = d1/2
def Ey_f(f):
...
FDs = bk.data([FD_Ey_f(f, eps) for eps in eps_vals])
#autodiffgrad = jax_grad(Ey_f)
c = Ey_f(f)
c.backward()

AD_grad = f.grad
#print(eps_vals[100], eps_vals[101],  FDs[100], FDs[101])
fig, ax = plt.subplots(1)
ax.plot(eps_vals.detach(), FDs.detach(), '-k')
ax.axhline(y=AD_grad, color='r', linestyle=':', label="Automatic derivative")
ax.set_xscale("log")
ax.set_yscale("symlog", linthresh=20)
